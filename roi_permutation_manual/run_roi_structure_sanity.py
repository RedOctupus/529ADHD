...
SEED = 42
GLOBAL_PERM_SEED = 123
SUBJECT_PERM_SEED = 456
...
RESULT_ROOT = ROOT / "modling" / "GCN" / "roi_structure_check"
...
def apply_global_permutation(x, perm):
    return x[:, :, perm, :][:, :, :, perm]


def apply_subjectwise_random_permutation(x, seed):
    rng = np.random.default_rng(seed)
    out = np.empty_like(x)
    perms = []
    for idx in range(x.shape[0]):
        perm = rng.permutation(x.shape[-1])
        out[idx, 0] = x[idx, 0][np.ix_(perm, perm)]
...
class BrainNetCNN(nn.Module):
    def __init__(self, n_roi, channels=(24, 48, 96, 192), dropout=0.4, use_batchnorm=True):
        super().__init__()
...
def run_single_fold(x_train_full, y_train_full, x_test, y_test, cfg, n_roi):
...
def run_cross_validation(x, y, participant_ids, cfg, condition_name, n_roi):
    output_dir = RESULT_ROOT / condition_name
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(x, y), start=1):
        ...
        fold_metrics, test_prob, history_df = run_single_fold(x_train, y_train, x_test, y_test, cfg, n_roi)
        print(
        ...
        fold_rows.append({"task": "Sex", "condition_name": condition_name, "fold": fold, **fold_metrics})
        history_df["fold"] = fold
        history_df["condition_name"] = condition_name
    ...
    summary_df["condition_name"] = condition_name
...
def main():
    ...
    x_original = fc_matrices[:, None, :, :].astype(np.float32)
    y_sex = df_all["Sex_F"].to_numpy(dtype=np.int64)
    participant_ids = df_all["participant_id"].to_numpy()

    global_perm = np.random.default_rng(GLOBAL_PERM_SEED).permutation(n_roi)
    x_global_perm = apply_global_permutation(x_original, global_perm).astype(np.float32)
    x_subject_perm, subject_perms = apply_subjectwise_random_permutation(x_original, SUBJECT_PERM_SEED)

    permutation_meta = {
        "n_roi": n_roi,
        "global_permutation_seed": GLOBAL_PERM_SEED,
        "subject_permutation_seed": SUBJECT_PERM_SEED,
        "global_permutation_head": global_perm[:20].tolist(),
        "subject_permutation_example_head": subject_perms[0][:20],
    }
    with (RESULT_ROOT / "permutation_meta.json").open("w", encoding="utf-8") as f:
        json.dump(permutation_meta, f, indent=2)

    conditions = {
        "original_layout": x_original,
        "global_fixed_permutation": x_global_perm,
        "subjectwise_random_permutation": x_subject_perm,
    }

    comparison_rows = []
    for condition_name, x in conditions.items():
        print(f"\n=== Running {condition_name} ===")
        fold_df, summary_df = run_cross_validation(x, y_sex, participant_ids, CFG, condition_name, n_roi)
        summary_map = {
    ...
    comparison_path = RESULT_ROOT / "roi_structure_comparison.csv"
